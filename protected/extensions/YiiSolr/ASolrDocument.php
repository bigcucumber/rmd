...
class ASolrDocument extends CFormModel implements IASolrDocument{
     ...
     * @return ASolrDocument[] the documents found.
     */
    public function findMutiAll(ASolrCriteria $criteria = null)
    {
        Yii::trace(get_class($this).'.findMutiAll()','packages.solr.ASolrDocument');
        if ($criteria === null) {
            $criteria = new ASolrCriteria();
        }
        $count = $this -> count(new ASolrCriteria(array("query" => '*')));
